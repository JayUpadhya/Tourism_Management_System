...
const Salary = require("../../model/Salary/salaryModel");
...
function Calculation(basicSalary, attendance) {
  ...
  if (basicSalary > 0 && attendance >= 20) {
    ...
    data.netSalary =
      basicSalary - data.tax + data.allowance - data.epf - data.etf;
...
router.put("/:_id", async (request, response) => {
  try {
    ...
    // Calculate salary details
    const calculatedSalary = await Calculation(
      request.body.basicSalary,
      request.body.attendance
    );
    ...
    const result = await Salary.findByIdAndUpdate(_id, newSalary, {
      new: true,
    });
    ...
    return response
      .status(200)
      .send({ message: "Salary updated successfully", result });
  } catch (error) {
...
router.delete("/:_id", async (request, response) => {
  try {
    ...
    return response
      .status(200)
      .send({ message: "Salary deleted successfully" });
  } catch (error) {
    console.log(error.message);
    response.status(500).send({ message: error.message });
  }
