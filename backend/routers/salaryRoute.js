...
router.post('/', async (request, response) => {
    try {
        if ( !request.body.dic ||!request.body.nic || !request.body.name || !request.body.basicSalary || !request.body.attendance) {
            return response.status(400).send({
                message: 'Send all required fields: dic,nic,name, basicSalary, attendance',
            });
        }
